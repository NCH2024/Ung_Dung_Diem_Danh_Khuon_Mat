...
class ReportOfDay:
    def __init__(self, user_id, class_id, subject, lesson, schedule_date, main_ui):
        ...
        CTkLabel(master=self.report_window,
                 text=f"Xuất báo cáo điểm danh ngày: {self.schedule_date}.",
    ...
    def get_data_of_day(self):
        data = get_attendance_report_of_day(self.class_id, self.user_id, self.subject, self.lesson, self.schedule_date)
        if not data:
            return pd.DataFrame()
        data_format = {
            'STT': [],
        ...
        for index, row in enumerate(data):
            ...
            data_format['Thời gian điểm danh'].append(row[2])
        return pd.DataFrame(data_format)

    def import_data(self):
        df = self.get_data_of_day()
        if df is not None and not df.empty:
            with pd.ExcelWriter(self.entry_path.get(), engine='xlsxwriter') as writer:
                ...
                worksheet.merge_range(f'A3:{end_col_letter}3', f'Buổi học: {self.lesson} - Ngày: {self.schedule_date}',subtitle_format)
                ...
                if CTkMessagebox(title="Thao tác thành công", message=f"Đã lưu file tại: {self.entry_path.get()}", icon="check", option_1="OK").get()=="OK":
                    self.cancel_report()
        else:
